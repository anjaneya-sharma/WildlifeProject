...
function Annotate() {
  ...
  const handleSave = async () => {
    const clusterCenters = clusterMap; 
    console.log(userid);
    console.log(id);
    try {
      const response = await fetch(`http://localhost:8000/images/BirdCount/${id}/${userid}/annotations`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 200) {
        ...
        alert('Failed to save changes: ' + errorData.detail);
      }
    } catch (error) {
